...
__status__ = "beta"
__revdate__ = "$Date: 2011-10-24 14:23:59 -0400 (Mon, 24 Oct 2011) $"
...
##Change Log
##11/26/2010 Changed logging to use the logging.config. The logging setup is 
##now done in logging_toneDetect.conf
...
CHANNELS = 1
reset = False
validTonesFile = "ini/validTones.ini"
ignoreTonesFile = "ini/ignoreTones.ini"
toneMapFile = "ini/toneMap.csv"
FIFOLen = int(4)
...
logging.config.fileConfig('ini/toneDetect_logging.conf')
...
def makeToneMapDict():
    ...
    for row in reader:
        x.setdefault(row[1], {}).setdefault(row[2], {})\
            .setdefault(row[3], {}).setdefault(row[4], {})['email']=row[5]
        x.setdefault(row[1], {}).setdefault(row[2], {})\
            .setdefault(row[3], {}).setdefault(row[4], {})['dept']=row[0]
    return(x)
...
def getAlertInfo( timestamp, tones, toneMapDict ):
    ...
    while len(tone) > 0 and toneDetected == True:
        ...
        if xtone > 0:
            ...
            # If the difference between the FFT frequency and the nearest standard paging 
            # frequency is more than >= .1% the correct standard paging frequency
            # is probably missing from the validTones file. Adding the correct 
            #standard paging tone to the valitTone files should solve the problem
            if toneErr >= .1:
                logger.info('%s  Possible tone translation error: xtone= %s stdTone = %s, \
                ToneError = %s ' % (timestamp, xtone, stdTone, toneErr))
            tabTones.append(stdTone)
            timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            logger.debug('%s  xtone = %s tcount = %s stdTone = %s ToneError = %s ' % \
                         (timestamp, xtone, tcount, stdTone, toneErr))
    ...
    if len(tabTones) == 3:
        try:
            ...
            timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            fw = open("log/dept.log", "a")
            fw.write('"%s","%s","%s","%s","%s" \n' % (timestamp, tabTones[0], tabTones[1], tabTones[2], dept))
            fw.close()
            tabTones = tabTones[3:]
        except KeyError:
            pass
    ...
    if len(tabTones)%2 == 0:
        ...
        while len(tabTones) > 0:
            try:
                ...
                timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                fw.write('"%s","%s","%s","%s" \n' % (timestamp, tabTones[0], tabTones[1], dept))
                tabTones = tabTones[2:]
            except KeyError:
...
def processRecording( timestamp,
    ...
    if PLATFORM == "win32":
        ...
        enc = subprocess.Popen([localLame, "--quiet", "-r", "-s", \
            str(sampleRate/1000.0), "--bitwidth", \
            str(swidth*8), "-m", CHANNELStr, tmpFile, mp3File])
    else:
        mp3File = "out/%s-%s.mp3" % (re.sub('\:', '.', timestamp), depts)
        enc = Popen([localLame, "--quiet", "-r", "-s", \
            str(sampleRate/1000.0), "--bitwidth", 
            str(swidth*8), "-m", CHANNELStr, "-", mp3File], stdin=PIPE)
        for frame in recordedSamples:
    ...
    if testMode == True:
        mailList.append(testEmail)
    logger.debug("There are %s emails to be sent" % (len(mailList)))
    if len( mailList ) > 0:
        mail( emailType, emailUser, emailPass, subject, mailList, "", mp3File)
    toneDetected = False
    logger.debug("Leaving processRecording")

def mail( emailType, emailUser, emailPass, subject, mailList, text, attach):
...
if __name__ == "__main__":
    ...
    freqArray = collections.deque([1000, 2000, 3000], FIFOLen)
    rms = -1

    # These just convert times to samples for easy checks below
    minToneSamples = int(minToneLength / sampleDuration)
    minVoiceSamples = int(minVoiceLength / sampleDuration)
    #maxVoiceSamples = maxVoiceLength / sampleDuration
    maxSilenceSamples = int(maxSilenceLength / sampleDuration)
    maxRecordSamples = int(maxRecordLength/sampleDuration)
    ...
    while 1:
        ...
        while not audio.empty():
            timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
#            if audio.qsize() > 50: 
#                logger.debug("audio.qsize = %s)" % (audio.qsize()))
            pkt = audio.get()
            freq = getFreq( pkt )
            ...
            # load current frequency into a FIFO queue that is FIFOLen long
            if freq != -1 or toneDetected == True or recording == True:
                freqArray.appendleft(freq)
                #Calculate the pseudo RMS value of those three frequencies
                rms = math.sqrt(((max(freqArray) - min(freqArray))**2)/FIFOLen)
                """
                ...
                Added voiceSamples This value should indicate how much talking has happened since
                the tone was detected. Because there can be long periods of silence or a lot of tones
            ...
            if rms >= 1:
                if toneDetected == True:
                    silenceSamples = 0
                    voiceSamples += 1
            elif sum(freqArray)/FIFOLen < 0 or rms == -1:
                if toneDetected == True:
                    silenceSamples += 1
            else:
                silenceSamples = 0
            ...
            if toneDetected == True and (voiceSamples > minVoiceSamples\
               and silenceSamples > maxSilenceSamples) or len(recordedSamples) > maxRecordSamples:
                ...
                if len(tones)> 0:
                    logger.debug("Before tone validation = %s " % (tones))
                    toneary = np.asanyarray(tones)
                    #Remove any nan's from the array
                    toneary = toneary[~np.isnan(toneary)]
                    #Get rid of any tones less than 280Hz
                    toneary = toneary[(toneary > 280)]
                    #If we are left with too few tones then abort
                    if len(toneary) <= minToneSamples:
                        toneDetected = False
                        reset = True
                        break
                    #change the tones back to a list 
                    tones = toneary.tolist()
                    logger.debug("After tone validation = %s " % (tones))
                    ...
                    for i in xrange(0,len(toneCount)):
                        toneCount[i] = toneCount[i] + FIFOLen
                    logger.debug("adjusted toneCount = %s" % (toneCount))
                    for i in reversed(xrange(0, len(toneCount))):
                        if toneCount[i] < 2:
            ...
            if toneDetected == True and recording == False and len(tones) > 0:
                ...
                processRecording( timestamp, tones, toneMapDict, recordedSamples, sampleRate, swidth, CHANNELS)
